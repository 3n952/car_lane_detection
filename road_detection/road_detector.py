...
def main(args):
    ...
    # 원본 프레임에 적용하기 위한 마스크 생성
    mask = gray_scale_road == 255 # boolean

    # 마스크 적용
    result = np.zeros_like(frame)
    for i in range(3):
        result[:,:,i][mask] = frame[:,:,i][mask]
        
    result = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)

    # 시각화 이미지 출력
    cv2.imshow("Result Image", result)  # 창 이름과 표시할 이미지 전달
